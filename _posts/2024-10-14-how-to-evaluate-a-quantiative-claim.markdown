...
Statistical uncertainty can be challenging to understand. I find it useful to think about a thought exercise: suppose we repeated the procedure used to generate the claim thousands of times. How much would the effect size change? What range would encompass most of the observed values?
...
The big problem with the “other uncertainty” category is that it is difficult to estimate by how much the uncertainty increases, and by how much our confidence should deteriorate. For example, we cannot estimate the changes caused by confounders that we cannot measure.
...
But more important than dissecting a particular claim is understanding the general process of evaluating a claim. Once again, there are four key steps:

1. **Rewrite** the claim as a quantitative one, and identify what **effect** is being computed, what **groups** are being compared, what **type** of claim (causal/counterfactual, or descriptive) is being posited, and what **evidence** is being marshalled.
