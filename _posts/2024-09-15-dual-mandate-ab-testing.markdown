...
The framework of “Type S” and “Type M” errors is useful here (see [Gelman and Carlin, 2014](http://www.stat.columbia.edu/~gelman/research/published/retropower_final.pdf)). Type S errors are those in which the _sign_ of the effect size estimate is wrong; Type M errors are those in which the _magnitude_ of the effect is exaggerated. In industry, we usually set statistical power at 80%: we want an 80% chance of detecting an effect of a particular size. But, according to this framework, even experiments with power much less than 80% can have reasonable type S and type M errors, particularly the former. For example, if power = 20%, the probability of a sign error is still only 0.005, and the “exaggeration ratio” is a relatively modest 2.3. The authors write,
